...
<main class="bg-colors2">
  ...
  <form id="AddData" method="POST" onsubmit="validateForm();" class="mt-10 font-bellota">
    ...
    <div class="relative z-0 w-full mb-6 group">
      <select name="lastName" id="lastName" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required />
    ...
    <div class="relative z-0 w-full mb-6 group">
      <select name="country" id="country" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" required>
        <?php foreach ($countries as $country) : ?>
          <?php echo "<option value='" . $country['name'] . "'>" . $country['name'] . "</option>"; ?>
        <?php endforeach; ?>
      </select>
    ...
    <div class="grid md:grid-cols-2 md:gap-6">
      ...
      <div class="relative z-0 w-full mb-6 group">
        <input type="text" name="message" id="message" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="What's the reason?" required />
        <label for="message" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-colors1 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-8">Message</label>
        <span class="error-message text-colors1" id="message-error"></span>
      </div>
    </div>
    <button type="submit" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border border-gray-300 appearance-none dark:text-white peer">Submit</button>
  </form>
</main>